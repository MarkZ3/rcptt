...
public class WriteExcelFileService implements ICommandService {
	...
	public IStatus service(Command command, IProcess context)
		...
		Workbook book;
		try {
			book = readBook(file, uri, isAppend);
	...
	private Workbook readBook(EclFile file, String uri, boolean isAppend) throws CoreException {
		Workbook book;
		if (isAppend) {
	...
	private void validateHeaders(Sheet sheet, Table table, EclFile file) throws CoreException {
		...
		if (headersRow.getLastCellNum() != tableHeaders.size()) {
			throw new CoreException(
					EclDataApachePOIImplPlugin
							.createErr("Table headers length from input does not match with headers length in the file",
									file.toURI()));
		}
		int cellnum = 0;
		for (String tableHeader : tableHeaders) {
			...
			if (headerCell == null
					|| !tableHeader.equals(ExcelFileService.getCellValue(headerCell))) {
				throw new CoreException(
						EclDataApachePOIImplPlugin
								.createErr("%s table header from input does not match with header %d in the file",
										tableHeader, cellnum, file.toURI()));
			}
			cellnum++;
		}
	}
